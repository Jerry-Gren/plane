...
void *hal_mmu_map_early_framebuffer(uint64_t phys_addr, uint64_t size) {
    uint64_t phys_base = ALIGN_DOWN(phys_addr, LARGE_PAGE_SIZE);
    uint64_t page_offset = phys_addr - phys_base;

    uint64_t fb_aligned_size = ALIGN(size + page_offset, LARGE_PAGE_SIZE);
    uint64_t pages_needed = fb_aligned_size / LARGE_PAGE_SIZE;

    for (uint64_t i = 0; i < pages_needed; i++) {
        uint64_t offset = i * LARGE_PAGE_SIZE;
